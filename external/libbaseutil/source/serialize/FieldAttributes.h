...
public:
    IFieldAttribute(int flag = ATTR_DEFAULT, SubType subType = SubType::Basic)
        : m_attrFlag(flag)
        , m_subType(subType){};
...
{
public:
    IntegerFieldAttribute(int64 rmin, int64 rmax, int64 s, bool spin = true, bool wrap = false, int flag = ATTR_DEFAULT)
...
{
public:
    FloatFieldAttribute(double rmin, double rmax, double s, bool spin = true, bool wrap = false, int flag = ATTR_DEFAULT)
        : IFieldAttribute(flag)
        , rangeMin(rmin)
        , rangeMax(rmax)
        , step(s)
...
public:
    ...
    {
    public:
        EnumItemInfo(int value, const tstring& name)
            : m_value(value)
            , m_name(name){};
...
public:
    EnumFieldAttribute(const EnumItemInfoList* pInfoList, int flag = ATTR_DEFAULT)
        : IFieldAttribute(flag)
...
public:
    ...
    {
    public:
        FlagItemInfo(uint value, const tstring& name, const tstring& displayName)
            : m_value(value)
            , m_name(name)
...
public:
    FlagFieldAttribute(const FlagItemInfoList* pInfoList, int flag = ATTR_DEFAULT)
        : IFieldAttribute(flag)
        , m_pInfoList(pInfoList){};
...
#define FLAG_ITEM(x, n)                                                                                                                                        \
    {                                                                                                                                                          \
        static_cast<int>(x), #x, n                                                                                                                             \
    }
