...
size_t FileUtil::listFilesInDirectory(StringSet& filesOut, const tstring& dir, const StringSet& includeExts /* = TS_STRING() */, const StringSet& excludeExts /* = TS_STRING() */)
{
...
tstring FileUtil::replaceFileExtension(const tstring& strIn, const tstring& newExt)
{
	...
	if (nPos != tstring::npos)
	{
		if (*newExt.begin() == '.') return strIn.substr(0, nPos) + newExt;
		else return strIn.substr(0, nPos) + '.' + newExt;
	}
		...
		while ((pEnt = readdir(pDir)) != nullptr)
		{
			if (pEnt->d_type == DT_DIR)
			{
				if (CURRENT_DIR != pEnt->d_name && PARENT_DIR != pEnt->d_name)
				{
					...
					numFilesFound += enumlateFiles(filesOut, currDir);
				}
			}
