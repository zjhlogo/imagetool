...
bool PackingUtil::pack(int& widthOut,
                       int& heightOut,
                       PieceFileInfoMap& pieceFileInfoMap,
                       const spank::tstring& strFileOut,
                       int packingFlag /* = PACKING_FLAG_MAX_QUALITY */)
{
...
bool PackingUtil::repackImage(fipImage& imageOut, const PieceFileInfoMap& pieceFileInfoMap, int packingFlag)
{
    for (const auto& it : pieceFileInfoMap)
    {
        ...
        if (packingFlag & PACKING_FLAG_TRIM_WHITE_SPACE_BORDER)
        {
            ...
            srcImage.copySubImage(subImage,
                                  fileInfo.offsetX,
                                  height - (fileInfo.offsetY + fileInfo.height),
                                  fileInfo.offsetX + fileInfo.width,
                                  height - fileInfo.offsetY);
        }
        else
        {
