...
namespace rbp
{
    ...
    MaxRectsBinPack::MaxRectsBinPack(int width, int height) { Init(width, height); }
    ...
    bool MaxRectsBinPack::Insert(std::vector<RectSize>& rects, std::map<int, Rect>& dst, FreeRectChoiceHeuristic method)
    {
        dst.clear();

        std::vector<RectSize> rectsCopy = rects;

        std::vector<int> rectsIndex;
        for (int i = 0; i < (int)rectsCopy.size(); ++i)
        {
            rectsIndex.push_back(i);
        }

        while (rectsCopy.size() > 0)
        {
            ...
            for (size_t i = 0; i < rectsCopy.size(); ++i)
            {
                int score1;
                int score2;
                Rect newNode = ScoreRect(rectsCopy[i].width, rectsCopy[i].height, method, score1, score2);
            ...
            dst.insert(std::make_pair(rectsIndex[bestRectIndex], bestNode));

            rectsCopy.erase(rectsCopy.begin() + bestRectIndex);
            rectsIndex.erase(rectsIndex.begin() + bestRectIndex);

            if (progressCb) progressCb(fmt::format("remain: {0}", rectsCopy.size()));
        }
    ...
    Rect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int& bestY, int& bestX) const
    {
        ...
        for (size_t i = 0; i < freeRectangles.size(); ++i)
        {
            ...
            if (rotateEnable && freeRectangles[i].width >= height && freeRectangles[i].height >= width)
            {
    ...
    Rect MaxRectsBinPack::FindPositionForNewNodeBestLongSideFit(int width, int height, int& bestShortSideFit, int& bestLongSideFit) const
    {
        Rect bestNode;
        memset(&bestNode, 0, sizeof(Rect));
    ...
    bool MaxRectsBinPack::SplitFreeNode(Rect freeNode, const Rect& usedNode)
    {
        ...
        if (usedNode.x >= freeNode.x + freeNode.width || usedNode.x + usedNode.width <= freeNode.x || usedNode.y >= freeNode.y + freeNode.height
