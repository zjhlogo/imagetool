...
bool ImageTool::resizeImageByPixel(const spank::StringList& files, const spank::StringList& exts, int pixelWidth, int pixelHeight)
{
	...
	for (const auto& file : fileSet)
	{
		...
		{
			std::cout << "Skip scale image, already under size (" << pixelWidth << ", " << pixelHeight << ") image size = (" << oldWidth << ", " << oldHeight << ") " << file << std::endl;
			continue;
		}
		...
		{
			std::cout << "Error, scale " << scale << " image failed: (" << oldWidth << ", " << oldHeight << ")->(" << newWidth << ", " << newHeight << ") " << file << std::endl;
			continue;
		}
...
bool ImageTool::convert(const spank::StringList& files, const spank::StringList& exts, const spank::tstring& destType, int depth /* = -1 */, bool deleteSrc /* = false */)
{
...
bool ImageTool::replaceChannel(const spank::tstring& strFile1, FREE_IMAGE_COLOR_CHANNEL eChannel1, const spank::tstring& strFile2, FREE_IMAGE_COLOR_CHANNEL eChannel2, const spank::tstring& strOutFile)
{
	fipImage image1;
...
bool ImageTool::binPack(const spank::StringList& files, const spank::StringList& exts, const spank::tstring& outputImageType, const spank::tstring& strOutFile, int packingFlag)
{
	spank::StringSet fileSet;
	collectFiles(fileSet, files, exts);
	if (fileSet.empty()) return true;
...
bool ImageTool::splitByCount(const spank::StringList& files, const spank::StringList& exts, int rows, int cols, const spank::tstring& outputType, const spank::tstring& dirOut)
{
	spank::StringSet fileSet;
	collectFiles(fileSet, files, exts);
	if (fileSet.empty()) return true;
...
bool ImageTool::splitBySize(const spank::StringList& files, const spank::StringList& exts, int subWidth, int subHeight, const spank::tstring& outputType, const spank::tstring& dirOut)
{
	spank::StringSet fileSet;
	collectFiles(fileSet, files, exts);
	if (fileSet.empty()) return true;
...
bool ImageTool::groupByFingerPrint(const spank::StringList& files, const spank::StringList& exts, const spank::tstring& outputType, const spank::tstring& commandFormat, int distance)
{
	spank::StringSet fileSet;
	collectFiles(fileSet, files, exts);
	if (fileSet.empty()) return true;
	...
	while (!fileInfoList.empty())
	{
		...
		while (it != fileInfoList.end())
		{
			...
			if (currFileInfoGroup.fileList[0].width == testFileInfo.width
				&& currFileInfoGroup.fileList[0].height == testFileInfo.height
...
bool ImageTool::groupByName(const spank::StringList& files, const spank::StringList& exts, const spank::tstring& outputType, const spank::tstring& commandFormat, int distance)
{
	spank::StringSet fileSet;
	collectFiles(fileSet, files, exts);
	if (fileSet.empty()) return true;
...
void ImageTool::collectFiles(spank::StringSet& fileSet, const spank::StringList& files, const spank::StringList& exts)
{
	...
	for (const auto& ext : exts) extSet.insert(ext);
...
void ImageTool::saveAtlasInfoXml(const spank::tstring& id, const spank::tstring& imageFile, const spank::tstring& configFile, int width, int height, const PackingUtil::PieceFileInfoMap& pieceFileInfoMap)
{
    tinyxml2::XMLDocument doc;
...
void ImageTool::saveAtlasInfoCpp(const spank::tstring & strOutDir, const spank::tstring & className, const spank::tstring& imageType, float width, float height, const PackingUtil::PieceFileInfoMap & pieceFileInfoMap)
{
    ...
    for (auto it : pieceFileInfoMap)
    {
        ...
        auto strPieceListItem = fmt::format("        {{ \"{}\", {{ {}, {} }}, {{ {}, {} }}, {{ {}, {} }}, {{ {{ {:.03f}f, {:.03f}f }}, {{ {:.03f}f, {:.03f}f }}, {{ {:.03f}f, {:.03f}f }}, {{ {:.03f}f, {:.03f}f }} }} }},\n",
            fileInfo.strId,
            fileInfo.srcWidth, fileInfo.srcHeight,
            fileInfo.width, fileInfo.height,
            fileInfo.offsetX, fileInfo.offsetY,
            lb.x, lb.y,
            rt.x, lb.y,
            lb.x, rt.y,
            rt.x, rt.y);
