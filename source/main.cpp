...
{
	if (channel == "r") return FICC_RED;
	else if (channel == "g") return FICC_GREEN;
	else if (channel == "b") return FICC_BLUE;
	return FICC_ALPHA;
}
...
int main(int argc, char** argv)
{
	...
	else if (splitSwitch.isSet() && inputFileArg.isSet() && typeArg.isSet() && outputFileArg.isSet())
	{
		...
		{
			tool.splitByCount(inputFileArg.getValue(), extsArg.getValue(), rowsArg.getValue(), colsArg.getValue(), typeArg.getValue(), outputFileArg.getValue()[0]);
		}
		else if (widthArg.isSet() && heightArg.isSet())
		{
			tool.splitBySize(inputFileArg.getValue(), extsArg.getValue(), widthArg.getValue(), heightArg.getValue(), typeArg.getValue(), outputFileArg.getValue()[0]);
		}
	}
